...
static void print_link_metrics(void *ctx, int cpu, void *data, __u32 size)
{
    ...
    // status infos
    fprintf(stdout,
        "|%s_%s|%u|%u|%s|%s|%u|%u|%u|%u|%u|%u|%u|%u|%u|%u|%u|%u|%u|%u|%d|%d|%d|%d|%d|%d|%d|%d|%u|%u|%u|%u|%u|%u|\n",
        OO_NAME,
        OO_TYPE_STATUS,
        link->tgid,
        link->role,
        ...
        metrics->data.status.syn_srtt_last,
        metrics->data.status.syn_srtt_max,
        metrics->data.status.syn_srtt_min,
