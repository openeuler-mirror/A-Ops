...
struct bpf_map_def SEC("maps") endpoint_map = {
    ...
    .value_size = sizeof(enum endpoint_t),
...
static __always_inline int create_sock_map(struct sock *sk, enum endpoint_t type)
{
    return bpf_map_update_elem(&endpoint_map, &sk, &type, BPF_ANY);
}
...
static __always_inline int get_sock_type(struct sock *sk, enum endpoint_t *type)
{
    enum endpoint_t *pet = bpf_map_lookup_elem(&endpoint_map, &sk);
    if (pet == 0)
        return -1;
    *type = *pet;
    return 0;
...
static __always_inline void __do_report(struct pt_regs *ctx, struct endpoint_val_t *val)
{
	(void)bpf_perf_event_output(ctx, &output, BPF_F_ALL_CPU, val, sizeof(struct endpoint_val_t));
}

static __always_inline void report(struct pt_regs *ctx, struct endpoint_val_t *val, u32 new_entry)
{
	if (new_entry) {
		__do_report(ctx, val);
	} else {
